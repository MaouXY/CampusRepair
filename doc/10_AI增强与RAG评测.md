...
### 3.6 实测基线（2026-09-11，本机 MySQL + 真实检索链路）

内置评测集 `campus-repair-builtin`（8 条：7 条可回答 + 1 条拒答），`topK=5`，运行一次评测的实测结果：
...
未命中的 2 条（「教室门锁打不开」「投影仪没有信号」）经核对属于**知识库内容缺口**（现有切片里没有门窗类、投影设备类条目），不是检索链路故障——这正是评测模块的价值：把「感觉检索不准」变成可定位的待补知识清单；补齐这两类知识文档后重跑即可量化验证提升。

> 实测时的环境说明：Embedding 走火山方舟（Ark）接口，而该账号当前**欠费**（`AccountOverdueError`），embedding 调用失败后按设计自动降级为关键词检索，因此本次指标只反映关键词召回；同时 AI 预分析与月度报告的 AI 建议也会退化为规则兜底（`aiDegraded=true`）。充值或更换可用的 Embedding / LLM 供应商后，执行「知识管理 → 重建」让切片重新入向量库，再重跑评测即可对比混合检索（RRF）带来的提升。
